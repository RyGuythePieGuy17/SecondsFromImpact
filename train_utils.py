        ...
        if 'spatial_encStack' not in key:
            ...
            if mapped_suffix == None:
                if suffix.endswith('bias'):
                    ...
                    vit_dim = from_model_dict[f'encoder.layer.{layer_number}.attention.attention.query.bias'].shape[0]
                    if dim > vit_dim:
                        q_bias = F.interpolate(q_bias.view(1,1, vit_dim), size=dim, mode = 'linear', align_corners=False )
                        q_bias = q_bias.squeeze(0).squeeze(0)
                        k_bias = F.interpolate(k_bias.view(1,1, vit_dim), size=dim, mode = 'linear', align_corners=False )
                        k_bias = k_bias.squeeze(0).squeeze(0)
                        v_bias = F.interpolate(v_bias.view(1,1, vit_dim), size=dim, mode = 'linear', align_corners=False )
                        v_bias = v_bias.squeeze(0).squeeze(0)
                    ...
                    vit_dim = from_model_dict[f'encoder.layer.{layer_number}.attention.attention.query.weight'].shape[0]
                    if dim > vit_dim:
                        q_weight = F.interpolate(q_weight.view(1,1,vit_dim, vit_dim), size=(dim, dim), mode = 'bilinear', align_corners=False )
                        q_weight = q_weight.squeeze(0).squeeze(0)
                        k_weight = F.interpolate(k_weight.view(1,1,vit_dim, vit_dim), size=(dim, dim), mode = 'bilinear', align_corners=False )
                        k_weight = k_weight.squeeze(0).squeeze(0)
                        v_weight = F.interpolate(v_weight.view(1,1,vit_dim, vit_dim), size=(dim, dim), mode = 'bilinear', align_corners=False )
                        v_weight = v_weight.squeeze(0).squeeze(0)
                    
                    weights = torch.cat([q_weight, k_weight, v_weight], dim = 0)
            else:
                weights = from_model_dict[f'encoder.layer.{layer_number}.{mapped_suffix}']
                # if ('bias' in mapped_suffix or 'norm' in mapped_suffix) and '0' not in mapped_suffix:
                #     if weights
